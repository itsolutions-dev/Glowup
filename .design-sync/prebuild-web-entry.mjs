...
const rnwAlias = {
  ...
  setup(b) {
    b.onResolve({ filter: /^react-native$/ }, (a) =>
      b.resolve('react-native-web', { kind: 'import-statement', resolveDir: a.resolveDir }));
    b.onResolve({ filter: /^react-native\// }, (a) =>
      b.resolve(a.path.replace(/^react-native\//, 'react-native-web/dist/'), {
        kind: 'import-statement', resolveDir: a.resolveDir }));
  },
};
...
const nodeStubs = {
  name: 'node-stubs',
  setup(b) {
    b.onResolve({ filter: /^node:async_hooks$/ }, (a) => ({ path: a.path, namespace: 'stub' }));
...
const r = await esbuild.build({
  ...
  resolveExtensions: ['.web.mjs', '.web.js', '.web.jsx', '.web.ts', '.web.tsx',
                      '.mjs', '.js', '.jsx', '.ts', '.tsx', '.json'],
  loader: { '.js': 'jsx', '.ttf': 'dataurl', '.otf': 'dataurl', '.woff': 'dataurl',
            '.woff2': 'dataurl', '.png': 'dataurl', '.jpg': 'dataurl', '.gif': 'dataurl',
            '.svg': 'dataurl' },
  define: { 'process.env.NODE_ENV': '"development"', __DEV__: 'true' },
  banner: { js: PROCESS_SHIM },
  metafile: true, logLevel: 'warning',
});
const pkgs = [...new Set(Object.keys(r.metafile.inputs).filter((k) => k.includes('node_modules/'))
  .map((k) => { const p = k.split('node_modules/').pop().split('/'); return p[0].startsWith('@') ? p.slice(0, 2).join('/') : p[0]; }))];
