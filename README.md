...
| `apps/playground` | `@glowup/playground` | Private Expo presentation app that demos the library on iOS, Android and Web. |
...
only through the package name `@glowup/ui` — both directions are enforced by ESLint.

## What's in `apps/playground`

The presentation app: a private Expo app whose only job is to exercise `@glowup/ui` on iOS,
...
- **Imports**: inside a workspace use relative paths; the app reaches the library only as
  `import { Button } from "@glowup/ui"`. Deep imports (`@glowup/ui/src/...`, or a path into
...
- Tests: components are tested in `packages/ui` (`npm test -w @glowup/ui`); the playground
