...
- 4fbf66b: Colour sets: `ThemeProvider` now resolves its tokens from a palette rather than
...
- 48c6ab0: `Modal`'s `visible` prop is required.
...
- 3fccd18: Fix Divider's sizing, and stop the language selector falling back to Italian.
...
- 4994093: **Breaking: the navigators are gone. The library is navigation-agnostic.**
...
- acf4691: Stop publishing source maps. The tarball goes from 706 files / 2.54 MB unpacked to 406 files
...
- 1a5cfa2: Make the theme's public types self-contained, declare the `react-dom` peer, and give the
